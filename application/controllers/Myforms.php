    ...
    public function wtl($key)
    {
        ...
        if ($this->input->post('key')) {
            if ($this->input->post('key') == $key) {
				...
				$assigneds= $this->db->get('tblwebtoleadresponsibles')->result();
				foreach($assigneds as $assigned){
					array_push($responsibles, $assigned->staff_id);
				}
				if(count($responsibles)==0){
					array_push($responsibles, $form->responsible);
				}
				if(count($responsibles) > 0)
                ...
                if ($insert_to_db == TRUE) {
                    ...
                    if ($lead_id) {
						...
						foreach($responsibles as $assigned){
							$assign_data = array('lead_id' => $lead_id, 'staff_id' => $assigned);
