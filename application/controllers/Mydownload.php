...
class Mydownload extends CI_Controller
{
    ...
    public function file($folder_indicator, $attachmentid = '')
    {
		...
		#Start OrNsoft modification download select file custom field
		} else if ($folder_indicator == 'customfiles') {
			if (!(is_staff_logged_in() || is_client_logged_in() || is_admin())) {
               die('Access forbidden');
            }
			$this->db->where('id', $attachmentid);
            $attachment = $this->db->get('tblfiles')->row();
            ...
            $path = FCPATH . 'uploads/'.$attachment->rel_type. '/' . $attachment->rel_id . '/' . $attachment->file_name;
		#Start OrNsoft modification download select file custom field
        } else {
