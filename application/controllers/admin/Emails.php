...
class Emails extends Admin_controller
{
    ...
    public function index()
    {
        ...
        foreach ($this->progibos_base->get_available_languages() as $av_language) {
    ...
    public function email_template($id)
    {
        ...
        $data['available_languages'] = $this->progibos_base->get_available_languages();
    ...
    public function sent_smtp_test_email()
    {
        if ($this->input->post()) {
            ...
            $this->email->subject('Progibos SMTP setup testing');
            $this->email->message('This is test email SMTP from Progibos. <br />If you received this message that means that your SMTP settings is set correctly');
            if ($this->email->send()) {
