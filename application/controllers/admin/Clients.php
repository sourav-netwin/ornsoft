...
class Clients extends Admin_controller
{
    ...
    public function index()
    {
        ...
        if ($this->input->is_ajax_request()) {
            $this->progibos_base->get_table_data('clients');
        }
    ...
    public function client($id = '')
    {
        ...
        if ($id == '') {
            ...
            if ($this->input->is_ajax_request()) {
                $this->progibos_base->get_table_data('tickets', array(
                    'userid' => $id
                ));
            }
    ...
    public function contact($customer_id, $contact_id = '')
    {
        ...
        $data['customer_permissions'] = $this->progibos_base->get_contact_permissions();
        $data['title']                = $title;
    ...
    {
        $this->progibos_base->get_table_data('contacts', array(
            'client_id' => $client_id
        ));
    }
                        ...
                        foreach ($rows as $row) {
                            ...
                            for ($i = 0; $i < count($db_fields); $i++) {
                                ...
                                if (in_array($db_fields[$i], $required) && $row[$i] == '') {
                                    $row[$i] = '/';
                                } elseif ($db_fields[$i] == 'country' || $db_fields[$i] == 'billing_country' || $db_fields[$i] == 'shipping_country') {
                                    if ($row[$i] != '') {
    ...
    public function groups()
    {
        ...
        if ($this->input->is_ajax_request()) {
            $this->progibos_base->get_table_data('customers_groups');
        }
