 ...
 * @return mixed
 */
function _progibos_upload_error($error)
{
...
{
    if (isset($_FILES['file']) && _progibos_upload_error($_FILES['file']['error'])) {
        header('HTTP/1.0 400 Bad error');
        echo _progibos_upload_error($_FILES['file']['error']);
        die;
    }
...
{
    if (isset($_FILES['company_logo']) && _progibos_upload_error($_FILES['company_logo']['error'])) {
        set_alert('warning', _progibos_upload_error($_FILES['company_logo']['error']));
        return false;
    }
...
function handle_project_discussion_comment_attachments($discussion_id, $post_data, $insert_data)
{
    ...
    if (isset($_FILES['file']['name']) && _progibos_upload_error($_FILES['file']['error'])) {
        header('HTTP/1.0 400 Bad error');
        echo json_encode(array('message'=>_progibos_upload_error($_FILES['file']['error'])));
        die;
    }
