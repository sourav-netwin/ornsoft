...
function app_stylesheet($path,$filename){
    ...
    return '<link href="'.base_url($path.'/'.$filename.'?v='.wordwrap($latest_version,1,'.',true)).'" rel="stylesheet">'.PHP_EOL;
}
/**
...
function app_script($path,$filename){
    ...
    return '<script src="'.base_url($path.'/'.$filename.'?v='.wordwrap($latest_version,1,'.',true)).'"></script>'.PHP_EOL;
}
/**
...
function init_relation_tasks_table($table_attributes = array())
{
    ...
    if ($table_attributes['data-new-rel-type'] == 'project') {
            echo "<a href='" . admin_url('tasks/list_tasks?project_id=' . $table_attributes['data-new-rel-id'] . '&kanban=true') . "' class='btn btn-default pull-left mbot25 mright5'>" . _l('view_kanban') . "</a>";
