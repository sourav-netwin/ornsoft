...
class Utilities_model extends CRM_Model
{
    ...
    public function get_calendar_data($client_id = '', $contact_id = '')
    {
        ...
        if (get_option('show_estimates_on_calendar') == 1) {
            ...
            foreach ($estimates as $estimate) {
                ...
                if (!$client_data) {
                    ...
                    //ornsoft change |--| BOF -- fix undefined error
                    //$rel_showcase = ' (' . $this->db->get('tblclients')->row()->company . ')';
                    $row = $this->db->get('tblclients')->row();
                    if (isset($row->company)){
                        $rel_showcase = ' (' . $row->company . ')';
                    }
                    //ornsoft change |--| EOF -- fix undefined error
                }
        ...
        if (get_option('show_tasks_on_calendar') == 1) {
            ...
            $this->db->select('duedate,startdate,name as title,id,rel_id,rel_type');
        ...
        $available_reminders = $this->progibos_base->get_available_reminders_keys();
        if (!$client_data) {
