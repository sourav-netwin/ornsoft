    ...
    public function get($id, $where = array())
    {
        ...
        if ($task) {
            ...
            if ($task->rel_type == 'project') {
                ...
                if ($task->milestone != 0) {
                    $task->milestone_name = $this->get_milestone($task->milestone)->name;
                }
            }
        }
        ...
        foreach ($checklists as $list) {
			...
			if($list['type'] == 4){
				if(isset($list['value']) && !empty($list['value'])){
					$sitems = @json_decode($list['value']);
					if(!is_null($sitems)){
						$new_values = array();
						foreach ($sitems as $val) {
							array_push($new_values, array('n'=>$val->n, 's'=>0));
						}
            ...
            $this->db->insert('tbltaskchecklists', array(
				...
				'type' => $list['type'],
				'value' => $value,
				'show_on_project' => $list['show_on_project'],
				//End OrNsoft modification task item
