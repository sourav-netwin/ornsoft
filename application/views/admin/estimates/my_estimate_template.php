    ...
    <div class="row">
        <div class="col-md-12 mtop15">
            <div class="panel-body">
                ...
                <?php if (isset($estimate) || ! has_permission('projects', '', 'create')) { ?>
                ...
                $('#estimate-form').validate({
                    ...
                    submitHandler: function(form) {
                        ...
                        if (create_type == 0) {
                            ...
                            $.post(admin_url + 'myestimates/estimate', $('#estimate-form').serialize(), function (data) {
                                var estimate = JSON.parse(data);
                                if (estimate && estimate.id && estimate.id > 0) {
                                    window.location.href = site_url + "admin/projects/project?customer_id=" + estimate.client_id + "&cost=" + estimate.total + "&est_id=" + estimate.id;
                                }
                            });
                                ...
                                if (estimate && estimate.id && estimate.id > 0) {
                                    $.get(admin_url + 'myleads/copy_project', function (response) {
                                        ...
                                        $('#copy_project').on('hidden.bs.modal', function () {
                                            window.location.href = admin_url + 'estimates/list_estimates/' + estimate.id;
                                        });
                                    });
                                }
                            });
