            ...
            <div class="col-md-3">
				...
				if($estimate->status == 2) {
					if($estimate->sent_date){
						$status_date = _dt($estimate->sent_date);
					} else {
						$this->db->like('additional_data', '<new_status>2</new_status>');
						$this->db->where(array('rel_type'=>'estimate', 'rel_id'=>$estimate->id));
						$activity = $this->db->get('tblsalesactivity')->row();
						if($activity)
							$status_date = _dt($activity->date);
					}
				}
				if($estimate->status == 4) {
					if($estimate->accepted_date){
						$status_date = _dt($estimate->accepted_date);
					} else {
						$this->db->like('additional_data', '<new_status>4</new_status>');
						$this->db->where(array('rel_type'=>'estimate', 'rel_id'=>$estimate->id));
						$activity = $this->db->get('tblsalesactivity')->row();
						if($activity)
							$status_date = _dt($activity->date);
				...
				?>
				<?php if($status_date != '') { ?>
				<a class="btn-with-tooltip" data-toggle="tooltip" title="<?php echo $status_date; ?>" data-placement="bottom">
				<?php } ?>
               ...
               <div class="pull-right _buttons">
                  ...
                  <div class="btn-group pull-right mleft5">
                     ...
                     <ul class="dropdown-menu">
                        <li><a href="<?php echo admin_url('estimates/convert_to_invoice/'.$estimate->id.'?save_as_draft=true'); ?>"><?php echo _l('convert_to_invoice_and_save_as_draft'); ?></a></li>
						<li><a href="<?php echo admin_url('estimates/convert_to_invoice/'.$estimate->id); ?>"><?php echo _l('estimate_convert_to_invoice'); ?></a></li>
                        <?php if($estimate->project_id == 0 && has_permission('projects', '', 'create')){ ?>
                        <li class="divider"></li>
						<li>
                           <a href="#" onclick="convert_to_project(<?php echo $estimate->id;?>); return false;"><?php echo _l('convert_to_project'); ?></a>
                        </li>
						<?php } ?>
                     </ul>
                  </div>
...
<script>
   ...
   function convert_to_project(est_id){
		$.get(admin_url + 'myleads/copy_project', function(response) {
			...
			$('#copy_form input[name="estimate_id"]').remove();
