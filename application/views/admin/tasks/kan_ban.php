...
foreach ($task_statuses as $status) {
  $total_pages = $this->tasks_model->do_kanban_query($status['id'],$this->input->get('search'),1,true,$where);
            ...
            <ul class="status tasks-status sortable relative" data-task-status-id="<?php echo $status['id']; ?>">
              ...
              foreach ($tasks as $task) {
                ...
                <li class="text-center not-sortable kanban-load-more" data-load-status="<?php echo $status['id']; ?>">
                 <a href="#" class="btn btn-default btn-block<?php if($total_pages <= 1){echo ' disabled';} ?>" data-page="1" onclick="kanban_load_more(<?php echo $status['id']; ?>,this,'tasks/tasks_kanban_load_more',270,360); return false;"; autocomplete="off" data-loading-text="<?php echo _l('wait_text'); ?>"><?php echo _l('load_more'); ?></a>
               </li>
               <?php } ?>
