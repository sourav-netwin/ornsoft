...
.add-items ul li a:hover,.add-items ul li a:focus{
	color:#004B6D;
}
.oplist-dialog{
	position:relative;
...
<div class="modal-body">
   <div class="row">
      <div class="col-md-8 task-single-col-left">
         ...
         <div class="clearfix"></div>
         <hr />
         <!--Start OrNsoft modification task item-->
		 <?php if(has_permission('tasks','','edit')) { ?>
		 <div class="add-items btn-group" id="task-item-menu" role="group">
			<button type="button" class="btn btn-default dropdown-toggle mright5" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
			  <i class="fa fa-plus-circle"></i> <?php echo _l('add_checklist_item'); ?>
			</button>
			<p class="no-margin pull-left">
				<a href="#" class="btn btn-default" id="lock-checkitems">
					<i class="fa fa-lock"></i>
				</a>
			</p>
			
			<ul class="dropdown-menu">
			  <li><a href="#" onclick="add_task_checklist_item('<?php echo $task->id; ?>'); return false"><i class="fa fa-plus-circle"></i> <?php echo _l('add_checklist_item'); ?></a></li>
			  <li><a href="#" onclick="add_task_datepicker_item('<?php echo $task->id; ?>'); return false"><i class="fa fa-plus-circle"></i> <?php echo _l('add_datepicker_item'); ?></a></li>
			  <li><a href="#" onclick="add_task_colorpicker_item('<?php echo $task->id; ?>'); return false"><i class="fa fa-plus-circle"></i> <?php echo _l('add_colorpicker_item'); ?></a></li>
			  <li><a href="#" onclick="add_task_select_file_item('<?php echo $task->id; ?>'); return false"><i class="fa fa-plus-circle"></i> <?php echo _l('add_select_file_item'); ?></a></li>
			  <li><a href="#" id="add-select-box"><i class="fa fa-plus-circle"></i> <?php echo _l('add_select_box_item'); ?></a></li>
			</ul>
		  </div>
		  <div class="oplist-dialog">
			  <div class="oplist-wrap">
				<h3><small><?php echo _l('select_box_option_list');?></small></h3>
				<div class="input-group">
					<input type="text" class="form-control" placeholder="New Option" name="option">
					<span class="btn btn-primary input-group-addon" id="add-option"><?php echo _l('select_box_add_option');?></span>
				</div>
				<ul class="list-unstyled" id="options"></ul>
				<div>
					<span id="opt-list-status" class="text-danger" style="font-size:13px; margin-left:5px;"></span>
					<button type="button" id="create-select-box" class="btn btn btn-primary pull-right"><?php echo _l('select_box_create');?></button>
				</div>
				<div class="clearfix"></div>
			  </div>
		  </div>
		  <?php } ?>
		 <!--End OrNsoft modification task item-->
...
<script>
   ...
   //Start OrNsoft modification task item
	$('#lock-checkitems').click(function(e){
		var locked = $(this).hasClass('btn-default');
		if(locked){
			$(this).removeClass('btn-default');
			$(this).addClass('btn-primary');
			$(this).find('i.fa').removeClass('fa-lock');
			$(this).find('i.fa').addClass('fa-unlock');
			...
			$("#checklist-items").sortable('enable');
		} else {
			$(this).removeClass('btn-primary');
			$(this).addClass('btn-default');
			$(this).find('i.fa').removeClass('fa-unlock');
			$(this).find('i.fa').addClass('fa-lock');
			$('.checklist-checkbox').each(function(){
				$(this).find('span.chl-desc').removeClass('hide');
				$(this).find('textarea[name="checklist-description"]').addClass('hide');
				$(this).find('a.remove-checklist').addClass('hide');
			});
			$("#checklist-items").sortable('disable');
		}
		return false;
	});
	$('#add-select-box').click(function(e){
		$('#task-item-menu').removeClass('open');
		$('.oplist-wrap').show();
		e.preventDefault();
		return false;
	});
	$('.oplist-wrap').click(function(e){
		e.preventDefault();
		return false;
	});
	$('#add-option').click(function () {
		var notExist = true;
		var optionName = $("input[name=option]").val().trim();
		if(optionName.length == 0) return;
		$('#options li.option-wrap span.option').each(function(){
			if($(this).html().trim()==optionName){
				notExist = false;
			}
		});
		if(notExist) $('#options').append("<li class='option-wrap'><span class='option'>" + optionName + "</span> <a href='#' class='pull-right text-danger remove-option'><i class='fa fa-remove'></i></a></li>");
		$("input[name=option]").val('');		
		$("input[name=option]").focus();
		$('#opt-list-status').html('');
	});
	$("input[name=option]").keypress(function(event) {
		  if(event.which == '13') {
			$('#add-option').click();
			$("input[name=option]").focus();
			return false;
    ...
    $(".oplist-wrap").on('click', 'li.option-wrap a.remove-option', function (){
        $(this).closest("li").remove();
	});
	$("#create-select-box").click(function(){
		var options = [];
		$('#options li.option-wrap span.option').each(function(){
			options.push({
				n:$(this).html().trim(),
		...
		if(options.length > 0){
			add_task_select_box_item(options);
			$('.oplist-wrap').hide();
		} else {
			$('#opt-list-status').html('<?php echo _l('select_box_create_error');?>');
		}
	});
	$("body").click(function(){
		$('.oplist-wrap').hide();
