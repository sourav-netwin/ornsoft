  ...
  <div class="content">
    <div class="row">
     ...
     <div role="tabpanel" class="tab-pane<?php if(!isset($form)){echo ' active';} ?>" id="tab_form_information">
       ...
       <?php } ?>
	   <?php
		$form_url = admin_url('myleads/form');
		if(isset($form)){
		 $form_url = admin_url('myleads/form/'.$form->id);
		}
		?>
       <?php echo form_open($form_url,array('id'=>'form_info')); ?>
...
<div class="col-md-12">
      ...
      <div class="col-md-6">
		...
		$rselecteds = array();
		if(isset($form)){
			$this->db->where(array('webtolead_id'=> $form->id));
			$rassigneds= $this->db->get('tblwebtoleadresponsibles')->result();
			foreach($rassigneds as $assigned){
				array_push($rselecteds, $assigned->staff_id);
			}
		 }
		 if(count($rselecteds)==0){
			 foreach($members as $staff){
			  if(isset($form) && $form->responsible == $staff['staffid']){
				array_push($rselecteds, $staff['staffid']);
			  }
			}
		 }
        ?>
        <?php echo render_select('responsible[]',$members,array('staffid',array('firstname','lastname')),'leads_import_assignee',$rselecteds,array('multiple'=>true)); ?>
        <hr />
