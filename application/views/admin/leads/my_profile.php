...
<hr class="no-margin" />
<?php
$form_url = admin_url('myleads/lead');
if(isset($lead)){
 $form_url = admin_url('myleads/lead/'.$lead->id);
}
?>
<?php if(isset($lead)){ ?>
...
<?php $selecteds = array();
 if(isset($lead)){
	$this->db->where(array('lead_id'=> $lead->id));
	$assigneds= $this->db->get('tblleadmembers')->result();
	foreach($assigneds as $assigned){
		array_push($selecteds, $assigned->staff_id);
	}
 }
 if(count($selecteds)==0){
	$selected = (isset($lead) ? $lead->assigned : get_staff_user_id());
	array_push($selecteds, $selected);
 }
 $assignees = '';
 $comma = '';
 foreach($selecteds as $assigned){
	$s_name = ($assigned != 0 ? get_staff_full_name($assigned) : '-');
	$assignees .= $comma . $s_name;
	$comma = ', ';
 }
 ?>
<p class="bold font-medium-xs mbot15"><?php echo $assignees ?></p>
...
<div class="col-md-4">
 ...
 if(isset($lead) && $is_assigned && $lead->addedfrom != get_staff_user_id() && !is_admin(get_staff_user_id())){
   $assigned_attrs['disabled'] = true;
 }
 echo render_select('assigned[]',$members,array('staffid',array('firstname','lastname')),'lead_add_edit_assigned',$selecteds,$assigned_attrs); ?>
</div>
<div class="clearfix"></div>
<hr class="no-mtop" />
<div class="col-md-6">
