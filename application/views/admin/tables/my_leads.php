...
$aColumns     = array(
    ...
    '(SELECT GROUP_CONCAT(staff_id SEPARATOR ",") FROM tblleadmembers WHERE lead_id=tblleads.id) as assigned',
...
if ($this->_instance->input->post('custom_view')) {
    ...
    if ($filter == 'lost') {
        ...
        array_push($where, 'AND (SELECT COUNT(id) FROM tblleadmembers WHERE lead_id=tblleads.id) = 0');
...
if(is_admin()){
    if ($this->_instance->input->post('assigned')) {
        ...
        array_push($where, 'AND ' . $by_assigned. ' IN (SELECT staff_id FROM tblleadmembers WHERE lead_id=tblleads.id)');
    }
}
...
if (!is_admin()) {
    ...
    array_push($where, 'AND (' . get_staff_user_id() . ' IN (SELECT staff_id FROM tblleadmembers WHERE lead_id=tblleads.id) OR addedfrom = ' . get_staff_user_id() . ' OR is_public = 1)');
}
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if($aColumns[$i] == '1'){
            ...
            $_data = '<a href="'.admin_url('leads/index/'.$aRow['id']).'" onclick="my_init_lead('.$aRow['id'].');return false;">'. $_data . '</a>';
