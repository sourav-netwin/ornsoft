...
$aColumns = array(
    ...
    'reference_no',
    'tblestimates.status',
    //'tblestimates.sent_date',
    //'tblestimates.accepted_date',
    );
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
		if ($aColumns[$i] == 'tblestimates.sent_date' || $aColumns[$i] == 'tblestimates.accepted_date') {
			continue;
		}
		...
		$status_date = '';
		if($aRow['tblestimates.status'] == 2) {
			if(!empty($aRow['tblestimates.sent_date'])){
				$status_date = _dt($aRow['tblestimates.sent_date']);
			} else {
				$CI =& get_instance();
				$CI->db->like('additional_data', '<new_status>2</new_status>');
				$CI->db->where(array('rel_type'=>'estimate', 'rel_id'=>$aRow['id']));
				$activity = $CI->db->get('tblsalesactivity')->row();
				if($activity)
					$status_date = _dt($activity->date);
			}
		}
		if($aRow['tblestimates.status'] == 4) {
			if(!empty($aRow['tblestimates.accepted_date'])){
				$status_date = _dt($aRow['tblestimates.accepted_date']);
			} else {
				$CI =& get_instance();
				$CI->db->like('additional_data', '<new_status>4</new_status>');
				$CI->db->where(array('rel_type'=>'estimate', 'rel_id'=>$aRow['id']));
				$activity = $CI->db->get('tblsalesactivity')->row();
				if($activity)
					$status_date = _dt($activity->date);
		...
		if($status_date != '') {
			$__data = '<a class="btn-with-tooltip" data-toggle="tooltip" title="'.$status_date.'" data-placement="bottom">'.format_estimate_status($aRow['tblestimates.status']).'</a>';
		} else {
