...
<div class="row">
  <div class="col-md-3">
   <?php if(has_permission('invoices','','create') || has_permission('invoices','','edit') || has_permission('invoices','','delete')){
  ...
  if($invoice->recurring > 0 || $invoice->is_recurring_from != NULL) {
    ...
    <div class="col-md-12">
      <h4 class="font-medium text-success">
        ...
        <?php echo '<i class="fa fa-question-circle" data-toggle="tooltip" data-title="Invoice will be recreated on specific hour of the day based from the setting located at Setup->Settings->Invoices->Cron Job"></i> ' . _l('next_invoice_date',_d($next_date)); ?><br />
        <?php if($invoice->status == 6){
          echo '<p class="mtop5"><small class="text-warning ">This invoice is with status draft, you need to mark this invoice as sent. Recurring invoices with status draft won\'t be recreated by cron job.</small></p>';
        }
        ?>
        <?php } ?>
        <?php
...
<div class="col-md-4 col-md-offset-8">
  ...
  <?php if($invoice->discount_percent != 0 && $invoice->discount_type != 'after_tax'){ ?>
  <tr>
   <td>
    <span class="bold"><?php echo ((isset($invoice->discount_name)&&$invoice->discount_name!='')?$invoice->discount_name:_l('invoice_discount')); ?> (<?php echo _format_number($invoice->discount_percent,true); ?>%)</span>
  </td>
  <td class="discount">
...
<?php if($invoice->discount_percent != 0 && $invoice->discount_type == 'after_tax'){ ?>
    <tr>
        <td align="right" width="85%"><strong><?php echo _l('invoice_subtotal_tax');?></strong></td>
        <td align="right" width="15%">-<?php echo format_money($invoice->subtotal + $invoice->total_tax, $invoice->symbol);?></td>
    </tr>
