...
$tbltotal .= '<table cellpadding="6" style="font-size:'.($font_size+5).'px">';
$tbltotal .= '
<tr>
...
if ($invoice->discount_type != 'after_tax'){
    if ($invoice->discount_percent != 0) {
        $tbltotal .= '
        <tr>
            <td align="right" width="85%"><strong>' . ((isset($invoice->discount_name)&&$invoice->discount_name!='')?$invoice->discount_name:_l('invoice_discount')) . ' (' . _format_number($invoice->discount_percent, true) . '%)' . '</strong></td>
...
if ($invoice->discount_type == 'after_tax'){
    if ($invoice->discount_percent != 0) {
        $tbltotal .= '
        <tr>
            <td align="right" width="85%"><strong>' . _l('invoice_subtotal_tax') . '</strong></td>
            <td align="right" width="15%">-' . format_money($invoice->subtotal + $invoice->total_tax, $invoice->symbol) . '</td>
        </tr>';
        $tbltotal .= '
        <tr>
            <td align="right" width="85%"><strong>' . ((isset($invoice->discount_name)&&$invoice->discount_name!='')?$invoice->discount_name:_l('invoice_discount')) . ' (' . _format_number($invoice->discount_percent, true) . '%)' . '</strong></td>
