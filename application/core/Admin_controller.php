...
class Admin_controller extends CRM_Controller
{
    ...
    public function __construct()
    {
        ...
        if (CI_VERSION != '3.1.3') {
            echo '<h2>Additionally you will need to replace the <b>system</b> folder. We updated Codeigniter to 3.1.3.</h2>';
            echo '<p>From the newest downloaded files upload the <b>system</b> folder to your ProgiBOS installation directory.';
            die;
        }
        ...
        if (!is_staff_logged_in()) {
            ...
            redirect(site_url('authentication/admin'), 'refresh');
        }
        ...
        if (!$this->input->is_ajax_request()) {
            if (ENVIRONMENT == 'production' && is_admin()) {
                if ($this->config->item('encryption_key') === '') {
                    die('<h1>Encryption key not sent in application/config/config.php</h1>For more info visit <a href="http://www.progibos.com/knowledgebase/encryption-key/">Encryption key explained</a> FAQ3');
                } elseif (strlen($this->config->item('encryption_key')) != 32) {
                    die('<h1>Encryption key length should be 32 charachters</h1>For more info visit <a href="http://www.progibos.com/knowledgebase/encryption-key/">Encryption key explained</a>');
                }
            }
        ...
        $auto_loaded_vars = array(
            ...
            '_quick_actions' => $this->progibos_base->get_quick_actions_links(),
    ...
    {
        $this->progibos_base->add_quick_actions_link(array(
        ...
        $this->progibos_base->add_quick_actions_link($tickets);
